...
<strong>Lectura del dataset employee_promotion.csv</strong>
```{r,eval=TRUE,echo=TRUE}
employee_promotion <- read.csv ("employee_promotion.csv")
head(employee_promotion,5)
```

<strong>Descripcion de dataset employee_promotion</strong>

```{r,eval=TRUE,echo=TRUE}
str(employee_promotion)

n_col <- ncol(employee_promotion)
n_row <- nrow(employee_promotion)
...
colnames(employee_promotion)[colSums(is.na(employee_promotion)) > 0]


cat("\n\n Nombre de NA en cada columna:  \n")
colSums(is.na(employee_promotion))
```
...
mutate(employee_promotion, employee_start_age = as.numeric(employee_promotion$age - employee_promotion$length_of_service))
head(employee_promotion,5)
```
...
min_age = min(employee_promotion$age)
max_age = max(employee_promotion$age)
lg_age = as.numeric(as.integer( (  max_age - min_age)/4) )

employee_promotion <- employee_promotion %>%
mutate(employee_promotion, age_group = ifelse(employee_promotion$age >= min_age & employee_promotion$age < (min_age + lg_age), "twenties",
 ifelse(employee_promotion$age >= (min_age + lg_age) & employee_promotion$age < (min_age + (2 * lg_age)), "thirties", 
  ifelse(employee_promotion$age >= (min_age + (2 * lg_age)) & employee_promotion$age < (min_age + (3 * lg_age)), "forties",
    ifelse(employee_promotion$age >= (min_age + (3 * lg_age)) & employee_promotion$age <= (min_age + (4 * lg_age)), "fifties", "Unclassified")))))
head(employee_promotion,5)
...
```{r,eval=TRUE,echo=TRUE}
library(plyr)
n_g_e <- ddply(employee_promotion, .(age_group), nrow)
n_g_e
...
employee_promotion <- employee_promotion[order(-employee_promotion$avg_training_score),]
max_value = employee_promotion$avg_training_score[1]

# Calcular los cuartiles de la dataset
max_rows = as.numeric(as.integer(nrow(employee_promotion)))
...
Q1_v = employee_promotion$avg_training_score[Q1_i]
Q2_v = employee_promotion$avg_training_score[Q2_i]
Q3_v = employee_promotion$avg_training_score[Q3_i]

employee_promotion <- employee_promotion %>%
mutate(employee_promotion, training_score_group = ifelse( is.na(employee_promotion$avg_training_score), "Abs",
 ifelse(employee_promotion$avg_training_score <= Q3_v, "D", 
  ifelse(employee_promotion$avg_training_score <= Q2_v, "C",
    ifelse(employee_promotion$avg_training_score <= Q1_v , "B", 
      ifelse(employee_promotion$avg_training_score <= max_value, "A", "Puntaje no clasificado"))))))
head(employee_promotion,5)
...
```{r,eval=TRUE,echo=TRUE}
library(plyr)
n_g_p <- ddply(employee_promotion, .(training_score_group), nrow)
n_g_p
```
...
```{r,eval=TRUE,echo=TRUE}
dep_age<- aggregate(age~department, employee_promotion, function(x) c(mean = mean(x), sd = sd(x)))
dep_age
```
...
data_means_dep_age <- aggregate(employee_promotion$age,                       # Means edad por department
                        list(employee_promotion$department),
                        mean)
data_means_dep_age
```
...
boxplot(employee_promotion$age ~ employee_promotion$department)                          
...
n_emp_total <- nrow(employee_promotion)
n_emp_total

#nombre de empleadores en cada departamento
n_emp_dep <- ddply(employee_promotion, .(department), nrow)
n_emp_dep
...
```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
mi.aov = aov(age~department,data = employee_promotion)
summary(mi.aov)
```
...
```{r,eval=TRUE,echo=TRUE}
ag2 <- aggregate(employee_start_age~gender, employee_promotion, function(x) c(mean = mean(x), sd = sd(x)))
ag2
```
...
```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
data_means <- aggregate(employee_promotion$employee_start_age,                       # Means por genero
                        list(employee_promotion$gender),
                        mean)
data_means
```
...
boxplot(employee_promotion$employee_start_age ~ employee_promotion$gender)                          
...
#nombre de observaciones total
n_obse_total <- nrow(employee_promotion)
n_obse_total

#nombre de observaciones para cada genero
n_eee <- ddply(employee_promotion, .(gender), nrow)
n_eee
...
```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
mi.aov = aov(employee_start_age~gender,data = employee_promotion)
summary(mi.aov)
```
...
```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
ep_PC=employee_promotion[,c("no_of_trainings", "age", "previous_year_rating", "length_of_service", "avg_training_score")]
