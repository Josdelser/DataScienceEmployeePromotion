...
Salama Mohamed-fadel Sidna:
  ...
  -Individual:
    ...
    La edad de incorporación del empleador en la compañía varía según el género? 
    Podemos reducir la dimensión, y que son los componentes que mas explica la variabilidad de nuestra dataset? 
...
<strong>la edad de incorporación del empleador en la compañía</strong>
```{r,eval=TRUE,echo=TRUE}
...
<strong>Recodificación de la variable "avg_training_score" en grupos</strong>
```{r,eval=TRUE,echo=TRUE}
...
La media de edad varia segunel departamento ? 
...
<li>boxplot de la variable employee_start_age para cada uno de los genero
```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
...
#nombre de observaciones para cada uno de los genero
...
Analisis de componentes principales creando un subconjunto de nuestra base de datos con las variables numéricas, y analizar sus correlaciones.
...
Para Interpretar los resultados, en primero si hay valor de correlacion positivo eso significa que los dos variables aumentan y bajan en paralelo.
...
Vemos que solamente con los tres primeros componentes podemos explicar el 96,77% de la variabilidad de la dataset, 
